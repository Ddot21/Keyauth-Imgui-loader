...
namespace KeyAuth {
	...
	public:
		...
		static std::string encrypt_string(const std::string& plain_text, const std::string& key, const std::string& iv) {
			...
			try {
				...
				MessageBoxA(NULL, ex.what(), "Failure", MB_OK);
				exit(0);
			}
			return cipher_text;
		}
		...
		static std::string decode(const std::string& encoded_text) {
			...
			try {
				...
				MessageBoxA(0, ex.what(), "KeyAuth", MB_ICONERROR);
				exit(0);
			}
	...
	public:
		...
		void init()
		{
			...
			{
				MessageBoxA(NULL, "Application Not Setup Correctly. Please Watch Video Linked in Main.cpp", "Failure", MB_OK);
				Sleep(4500);
				exit(0);
			}
			...
			{
				MessageBoxA(NULL, std::string(json[("message")]).c_str(), "Failure", MB_OK);
				exit(0);
			}
		}// 
